...
if ~isempty(find(strcmp(fields, 'orig'), 1));
    origFields = fieldnames(events(1).orig);
    if ~isempty(find(strcmp(origFields, 'durationRemainder'), 1));
        hasDurationRemainders = true;
    end
    if ~isempty(find(strcmp(origFields, 'sampleRemainder'), 1));
        hasSampleRemainders = true;
    end
    if ~isempty(find(strcmp(origFields, 'label'), 1));
        hasLabels = true;
    end
end
...
for p = 1:numEvents
    if isempty(events(p).value)
        ...
        theLabel = '';
        if hasLabels
            theLabel = events(p).orig.label;
        end
        event.setLabel(theLabel);
